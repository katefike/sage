...
```
15. While SSH'd to the droplet, verify that all your existing transactions (txns) are there. Connect to the DB by running `docker exec -it  sage-db psql -U <POSTGRES_USER> sage` and executing the query `SELECT * FROM txns;` `POSTGRES_USER` is an environment variable specified in your `.env`.