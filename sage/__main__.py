...
def main():
    ...
    if not load_dotenv(env_path):
        logger.critical(f"ENVIRONMENT ERROR: .env failed to load from \
            {env_path}")
    ...
    try:
        with imap_tools.MailBoxUnencrypted(IMAP4_FQDN).login(
            ...
            for msg in mailbox.fetch():
                ...
                if not msg.text or not msg.html:
                    logger.warning(f"Rejecting email from {msg.from_} because it \
                ...
                try:
                    ...
                    logger.info("FAILED")
                    logger.critical(f"EMAIL PARSER ERROR: Failed to parse msg \
                        UID {msg.uid}: {error}")
            ...
            deduced_total_messages_count = rejected_messages_count + \
                unparsed_messages_count + unwritten_transactions_count +\
                processed_transactions_count
            if deduced_total_messages_count != total_messages_count:
                logger.critical("FAILED")
                logger.critical(f"ERROR-HANDLING ERROR: {total_messages_count}\
            ...
            logger.info(f"Processed Transactions \
                {processed_transactions_count}")
            logger.info("DONE")
            return
        ...
        logger.critical("FAILED")
        logger.critical(f"MAILSERVER ERROR: Failed to connect via IMAP to the \
